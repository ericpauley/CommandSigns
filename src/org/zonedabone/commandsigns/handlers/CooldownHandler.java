		...
		if (e.getPlayer() != null && command.startsWith("~")) {
			...
			if (amount != 0) {
				...
				if ((latest == null) ^ negate) {
					...
					if (!silent) {
						if (negate)
							e.getPlayer().sendMessage(ChatColor.RED + "You must use this sign before you can use this sign... I'm confused.");
						else
							e.getPlayer().sendMessage(ChatColor.RED + "You can only use this sign once.");
					}
					if (negate) {
						lastUse.put(e.getPlayer().getName(), System.currentTimeMillis());
						latest = lastUse.get(e.getPlayer());
